...
app.get('/', function(req, res){
  ...
  res.snedFile(__dirname+ '/public/projector.html');
});
var tempData;
var ports=[]; 
var stepsize = 0.05;
...
function makeData() {
	...
	var t1 = d1.getTime();
	for(var i = 0; i < numsteps_cw;i++) {
		cw_val[i] = Math.round(128 + 40*Math.sin(2*0.4*t[i]*Math.PI));
		// console.log(t[i]);
...
// For the pi   
 var ports = [
 	{id: 'piezo', port: "/dev/ttyUSB0"}, // need to add the USB port for this arduino
 	{id: 'button', port: "/dev/ttyUSB1"}, // need to add this arduinos USB port
 	{id: 'pd', port: "/dev/ttyUSB2"}, // need to add this arduinos USB port
 ];
...
var boards = new five.Boards(ports);
	...
	// Boards = new five.Boards(ports).on("ready", function(Boards){ // multiple boards, make sure the ports are specified correclty
		...
		this.byId("button").pinMode(9,five.Pin.PWM);
		this.byId("button").pinMode(6,five.Pin.PWM);
		// Button ports
