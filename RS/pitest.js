...
// For the pi   
 var ports = [	// Check with Phil what the device locations are.
 	{id: 'button', port: "/dev/ttyUSB1"}, // need to add the USB port for this arduino
 	{id: 'piezo', port: "/dev/ttyACM0"} // need to add this arduinos USB port
 	{id: 'pd', port: "/dev/ttyUSB0"}
 ];
...
var boards = new five.Boards(ports);
	...
	// Boards = new five.Boards(ports).on("ready", function(Boards){ // multiple boards, make sure the ports are specified correclty
		...
		this.byId("pd").analogRead(4,function(data){ // Read data aas fast as we can and set it to a global (could be nicer )
		...
		this.byId("button").pinMode(11,five.Pin.PWM);	// Green LED
		this.byId("button").pinMode(9,five.Pin.PWM); 	// Blue LED
		this.byId("button").pinMode(10,five.Pin.PWM);	// White LED
		this.byId("button").pinMode(6,five.Pin.PWM);	// Yellow LED


//	We're commenting this out because we think this is the cause of the slowddowns.		
...
//	This version of pulse LED just sets them to be on, this is intended as a fallback. 


		function pulseLED() {
			// This is for pulsing all the LED's but we'll just set them to maximum
			boards.byId("button").analogWrite(6,255);
			boards.byId("button").analogWrite(9,255);
			boards.byId("button").analogWrite(10,255);
			boards.byId("button").analogWrite(11,255);
		}

// Need to think about how to get an exit condition for this and to prevent lag.
		// function pulseLED() {
		// 	for (j=0;j<numsteps_cw/10;j++) {
		// 	boards.byId("button").analogWrite(6,cw_val[j]);
		// 	boards.byId("button").analogWrite(9,cw_val[j]);
		// 	boards.byId("button").analogWrite(10,cw_val[j]);
		// 	boards.byId("button").analogWrite(11,cw_val[j]);
		// 	}

		// }

		function stopLED() {
			boards.byId("button").analogWrite(6,0);
			boards.byId("button").analogWrite(9,0);
			boards.byId("button").analogWrite(10,0);
			boards.byId("button").analogWrite(11,0);			
		}


// We only use these if we're setting the LED's up as LED's
		// function pulseLED() {
